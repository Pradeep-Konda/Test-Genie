Feature: Health Check Endpoint
  ...
  Scenario: Health check returns service is healthy (Happy Path)
  ...
  Scenario: Health check endpoint - injection attack attempt (Security)
  ...
  Scenario: Health check endpoint - excessive requests (Performance)
    Given the service is running
    When I send 100 GET requests to "/api/health" in 1 second
