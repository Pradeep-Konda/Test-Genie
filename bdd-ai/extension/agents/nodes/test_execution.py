...
#             "--------------------------\n"
#             "A scenario is:\n"
#             "- PASSED only if ALL checks pass\n"
...
from agents.utils.auth_handler import AuthHandler
from agents.utils.schema_validator import SchemaValidator
from agents.utils.report_handler import ReportHandler
...
class TestExecutionNode:
    ...
    async def _execute_scenario(self, scenario, state, resources, base_url):
        ...
        try:
            method, url, body = common.extract_http_call(full_scenario)
            expectations = common.extract_expected_status(full_scenario)
            content = common.get_content_from_spec(
            ...
            status_passed = common.validate_status(
            ...
            return {
                ...
                "url": url,
                "method": method,
                "status": HTTPException(status_code=500),
                "response": str(e),
                "result": "failed",
            }
    ...
    # --------------------------------------------------------
    async def __call__(self, state, resources):
        try:
            ...
            base_url = common.get_base_url_from_spec(state.analysis)
