...
class TestExecutionNode:
    ...
    def __init__(self, features_dir: str = "bdd_tests"):
        ...
        # ---------------------
        self.system_prompt = (
            "You are an API Test Executor. Execute Gherkin scenarios via TestExecutor and return JSON results.\n"
            "\n"
            "WORKFLOW:\n"
            "1. Get BASE_URL from OpenAPI spec's servers[0].url\n"
            "2. For each scenario: parse WHEN step for method, URL, body\n"
            "3. Call TestExecutor(method, url, base_url, body)\n"
            "4. Evaluate THEN assertions against response\n"
            "5. Return JSON results\n"
            "\n"
            "STATUS VALIDATION:\n"
            "- \"status should be 200\" → pass if actual == 200\n"
            "- \"status should be 200 or 204\" → pass if actual in [200, 204]\n"
            "- \"should succeed\" → pass if 200-299\n"
            "- \"should fail\" → pass if 400-599\n"
            "- No status assertion → auto-pass\n"
            "\n"
            ...
            "\n"
            "RULES:\n"
            "- Use exact URLs from spec — never invent or modify\n"
            "- Include ALL fields in every result object\n"
            "- \"result\": \"passed\" only if ALL assertions pass\n"
            "- On any error: return result with \"result\": \"failed\"\n"
        )
